...
//--MM: If this site doesnt use composer, I would expect a manually created autoloader that utilizes spl_autoload or spl_autoload_register to be present somehwere in the codebase, again, likely in a globally included file.  
//--MM: At its core, PSR-0/4 is essentially a group of rules that defined a standard set of naming conventions for directory and file structure. If these rules are met in given codebase, a psr0/4 compliant autloader can be ultilized. Composer being the most popular. 
...
class Routes extends \Dsc\Routes\Group
{
    ...
    public function initialize()
    {
        ...
        //--MM: First, I would expect the  \Dsc\Routes\Group base class to have a static method/facade proxy (via __call proxy), to implement an intialize() method that accepts some default meta parameters defined by the child class (this class).
        ...
        //--MM: Although the flexabilty has grown susstancially over the years, the core concept remains the same.  
        ...
        //--MM 'url_prefix' => Usually used for semantic versioning of API's. Not the only use case, but the most common in my experience. Could also be used to make url's more human readable if you have a very long unpredicable, or othwerwise difficult to understand resource name. 
        //--MM: Finally, I would add that all routes defined in this file will have the above two rules applied.
        //--MM: eg. The Address controller exists at \Shop\Sites\Controllers\Address, and if a url_prefix was defined here, it would be prepended to all routes, even if there route delcaration defines thier own url prefix. 
        $this->setDefaults(array(
            'namespace' => '\Shop\Site\Controllers',
            'url_prefix' => ''
        ));
        //TODO :: What do you think is happening here? 
        //--MM: I would his codebase is using some kind of MVC paradaigm. 
        //--MM: This is probably a definition of what should happen when the site recieves a GET request at <vhostRoot>/.
        //--MM: Specifially, when a GET request comes into <vhostRoot>/ it should be handled by a Controller class that lives  at \Shop\Site\Controllers\Home. 
        ...
        //--MM: These classes generally allow for creating, reading, updating, and delelting of a specific resource, or related group of resources. 
        //--MM: The defininition of "Resource" classes generally service one of two purposes. 
        //--MM: Allow for a CLI tool to assist with boilerplate/scaffolding. eg. If i know i want to have full CRUD functoinality, a tool like artisan can scaffold out the bare bones methods.
        //--MM: Validation, If i know a certain controller class should expose full Crud functionality, i will treat it as a contract/interface of sorts, and ensure all required methods are implmented. 
        //--MM: I would think this definition serves one of those two purposes. 

        //--MM: I was in the audiance when Adam Waltham gave this talk in 2017, and i belive its one of the best talks ever given on the subject in the context of PHP.
        //--MM: Weather you've been on the job for 10 years, or 10 mniuets, i believe anyone can get a bit of value out of that talk. 
